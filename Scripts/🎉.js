...
const template = `<ul class="cdc_flip cdc_dgtPlay">
    <li class="cdc_before">
      <div class="cdc_up">
...
function Init(n) {
	...
	for (var i = 0; i < n_s_before.length; i++) {
		html += template.replaceAll("cdc_dgtPlay", "cdc_" + i + "Play")
			...
			.replaceAll("dgt_active", parseInt(n_s.charAt(i)));
	}
...
function Play() {
	...
	if (minus1.length < n_s.length) 
		$(".cdc_container .cdc_" + (n_s.length - minus1.length - 1) + "Play").hide('slow');
	
	for (var i = minus1.length-1; i >= 0; i--) {
		var n_s_i = i + (n_s.length - minus1.length);
		if (n_s.charAt(n_s_i) != minus1.charAt(i)) {
	...
	n_s = minus1.padStart(n_s.length, '0');
...
function Timefy(n) {
	...
	for (var i = 0; i < n_s.length; i++) {
		var d = parseInt(n_s.charAt(i));
		if ((n_s.length - i) % 2 == 0 && d > 5) 
			d = 5;
		n = n * 10 + d;
	}
	return ('' + n);
